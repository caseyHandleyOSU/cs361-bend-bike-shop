...
class Bike
  ...
  def initialize(id, color, price, weight = STANDARD_WEIGHT_LBS, rented = false)
    ...
    @rentalData = Rentable.new(rented, price, 0) # Rentable class is used to extract data that may not be exclusive to bike rentals, without needing to change the [bikeInstance].rent! signature
    @cargo = Inventory.new([], 10) # A bike's cargo is basically just an inventory, which could be used elsewhere (such as storing the catalog of bikes!)
  end
